...
?>
		...
		echo $this->dropdownHelper->display(
			...
			$dropData,
			array('type' => 'link', 'right' => true, 'up' => false, 'hkicon' => 'icon-32-go-cart')
		);
			?></td>
<?php
	}
...
<?php
	if(hikashop_level(2) && !empty($this->productFields)) {
		foreach($this->productFields as $fieldname => $field) {
			echo '<th class="hikashop_cart_product_'.$fieldname.'">'.$this->fieldsClass->trans($field->field_realname).'</th>';
		}
	}
?>
...
<?php
	if(hikashop_level(2) && !empty($this->productFields)) {
		foreach($this->productFields as $field) {
			$namekey = $field->field_namekey;
?>			<td data-title="<?php echo $field->field_realname; ?>" >
<?php
...
?>
	...
	<div class="hikashop_product_quantity_main">
	<div class="jsn_quantity_default">			
<?php
	if(!empty($this->manage)) {
		if($this->cart->cart_type == 'wishlist') {
