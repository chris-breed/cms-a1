...
class SunFwSite
{
	...
	public function finalizeOutput()
	{
		...
		// Minify
		if ($this->app->isClient('site') && SunFwRecognization::detect())
		{
			//move Script Tags
	...
	public static function minifyHTML($body)
	{
		...
		$body = str_replace('$', '&dollar;', $body);

		$regex = "#(?><?[^<]*+(?><pre\b[^>]*+>(?><?[^<]*+)*?</pre\s*+>|<script\b[^>]*+>(?><?[^<]*+)*?</script\s*+>|<style\b[^>]*+>(?><?[^<]*+)*?</style\s*+>|<textarea\b[^>]*+>(?><?[^<]*+)*?</textarea\s*+>|<!--\[(?><?[^<]*+)*?<!\s*\[(?>-?[^-]*+)*?--!?>|<!DOCTYPE[^>]++>)?)*?\K(?:<!--(?>-?[^-]*+)*?--!?>|$)#i";
		$body = preg_replace($regex, '', $body);

		// Remove empty lines (between HTML tags); cannot remove just any line-end characters.
		$regex = '/\>[\r\n\t ]+\</s';
		$body = preg_replace($regex, "><", $body);
		...
		// Replace newlines in style tags with a single space.
		$body = str_replace('&dollar;', '$', $body);
