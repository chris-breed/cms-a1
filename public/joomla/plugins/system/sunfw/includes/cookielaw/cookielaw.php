...
class SunFwCookielaw
{
	...
	public static function loadCookie()
	{
		...
		if (@count($cookieLawData) && isset($cookieLawData['enabled']) && (int) $cookieLawData['enabled'])
		{
			...
			// The message...
			if (isset($cookieLawData['message_type']) && $cookieLawData['message_type'] == 'article' && isset($cookieLawData['article']) &&
				 !empty($cookieLawData['article']))
			{
				...
				elseif (isset($cookieLawData['cookie-policy-link']) && !empty($cookieLawData['cookie-policy-link']))
				{
